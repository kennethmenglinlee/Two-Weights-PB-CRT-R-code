...
```{r * sim PB-CRT data [additional EME biased scenario]}
library(dplyr)
library(lme4)
library(WeMix)
...
for(n in 1:n_sims){
  ...
  #     add_row(
  #       cluster_i=as.numeric(i),
  #       IEE = summary(lm(outcome ~ intervention + period, data = jk.dat))$coef[,"Estimate"]["intervention"],
  #       IEEw = summary(lm(outcome ~ intervention + period, data = jk.dat, weights = jk.dat$W1))$coef[,"Estimate"]["intervention"],
  #       FE = summary(lm(outcome ~ intervention + period + cluster, data = jk.dat))$coef[,"Estimate"]["intervention"],
  #       FEw = summary(lm(outcome ~ intervention + period + cluster, data = jk.dat, weights = jk.dat$W1))$coef[,"Estimate"]["intervention"],
  #       EME = summary(lme4::lmer(outcome ~ intervention + period + (1|cluster), data = jk.dat))$coef[,"Estimate"]["intervention"],
  #       EMEw = summary(mix(outcome ~ intervention + period + (1|cluster), data=jk.dat, weights=c("W1", "W1")))$coef[,"Estimate"]["intervention"],
  #       NEME = summary(lme4::lmer(outcome ~ intervention + period + (1|cluster) + (1|cluster:period), data = jk.dat))$coef[,"Estimate"]["intervention"],
  #       NEMEw = summary(mix(outcome ~ intervention + period + (1|cluster) + (1|cluster:period), data=jk.dat, weights=c("W1", "W1", "W1")))$coef[,"Estimate"]["intervention"]
  #     )
  #   print(paste0(nrow(sim.df),".",i))
  # }
  # jk.df
...
# sim.df
# write.csv(sim.df, paste0("results_",n_sims,"sim_",hetTrtmnt,"hettrtmnt_",CAC,"CAC_",ICC,"ICC_Scenario2_p1_",P1,"xi_",100*xi,"_100",".csv"))
```
...
Results are the same for pATE and cATE since estimands coincide
